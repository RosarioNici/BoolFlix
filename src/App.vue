  ...
  <div id="app">
    <header class="d-flex justify-content-between align-items-center">
      <div>
        ...
        <div class="header-container">
          <input type="text" v-model="call" @keyup.enter="callApi">
    ...
    <main>
        ...
        <div class="d-flex flex-wrap justify-content-around ">
          <moviesComponent v-for="film in movies" :key="film.id" :film="film"/>
        </div>
        <h1>Serie TV</h1>
        <div class="d-flex flex-wrap">
          <seriesComponent class="col-4" v-for="serie in series" :key="serie.id" :serie="serie"/>
        </div>
    </main>
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        callApi(textInput) {
          axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=ritorno+al+futuro&language=it-IT`)
                .then((response) => {
